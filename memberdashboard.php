...
<html>
  ...
  <body>
    ...
    <div class="page">
	 ...
	 <?php 
			// $user_id=$_SESSION['id'];
			// $member_query = mysqli_query("select * from user where user_id = '$user_id'")or die(mysql_error());
	  ...
	  <form method="post" action="post.php"> 
					...
					<hr>
						...
						<?php	
							// $sqlpost="SELECT *,UNIX_TIMESTAMP() - date_created AS TimeSpent from post LEFT JOIN user on member.ID = post.user_id order by post_id DESC";
						...
						<form method="post" action="comment.php">
                            ...
                            <div>
                              <span>
                                <a href='<?=$str2?>' > <?=$name2?> </a>
                                <span>(<?php				
								// $days = floor($row3['TimeSpent'] / (60 * 60 * 24));
								// $remainder = $row3['TimeSpent'] % (60 * 60 * 24);
								// $hours = floor($remainder / (60 * 60));
								// $remainder = $remainder % (60 * 60);
								// $minutes = floor($remainder / 60);
								// $seconds = $remainder % 60;
